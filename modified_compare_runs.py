...
def plot_three_psfs(one_fit_dir, two_fit_dir, three_fit_dir, one_img_root, two_img_root, three_img_root, mode):
    ...
    plt.savefig('psf_compare.png', dpi=500, facecolor='w', bbox_inches='tight')
    plt.show()
    return
...
def compare_fvu_on_three_star(one_fit_dir, two_fit_dir, three_fit_dir, one_img_dir, two_img_dir, three_img_dir, one_img_root, two_img_root, three_img_root, mode, star_name, star_pos, star2_pos, star3_pos):
    ...
    res_cut_one = Cutout2D(oneres, c2_pos, c_siz)
    img_cut = Cutout2D(imag, c_pos, c_siz)
    ...
    idx_one = np.where((sl_one['x'] > c2_pos[0]-5) & (sl_one['x'] < c2_pos[0]+5) &   #DEFAULT
                       (sl_one['y'] > c2_pos[1]-5) & (sl_one['y'] < c2_pos[1]+5))[0] #Although hard-coded c2_pos
    #idx_one = np.where((sl_one['x'] == c_pos[0] + 27.99) &  #Only use if reference list has LARGE offset (> 5 pixels)
    #                   (sl_one['y'] == c_pos[1] + 29.88))
    ...
    np.savetxt(star_name+'.fvu', [(air_fvu_one,air_fvu_two,air_fvu_three)], fmt='%1.5f', header="poor average best")
...
def compare_fvu_vs_mag(old_fit_dir, new_fit_dir, old_img_dir, new_img_dir, old_img_root, new_img_root, mode):
    ...
    plt.semilogy(sl_new['m'][idx_new], fvu_old, 'b+', label='Old', alpha=0.99)
    plt.semilogy(sl_new['m'][idx_new], fvu_new, 'rx', label='New', alpha=0.99)
    plt.legend()
